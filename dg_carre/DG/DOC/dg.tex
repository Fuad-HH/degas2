...
\begin{document}
\title{\includegraphics[height=12.0cm]{nstx_22_dg.pdf} \\
...
are possible.  The flexible design of \textbf{DG} allows its
output to be used with multiple magnetic fusion simulation codes,
including \textbf{B2-Eirene} and \textbf{DEGAS 2}.  In fact, some
of the objects defined in \textbf{DG} and described in this manual
...
\textbf{DG }is the backbone of the \textbf{Interactive Graphical
Interface} package. It allows user to visualize the model geometry, to
modify it, and to specify the geometry-related data necessary for a
grid generator and modelling codes. It was originally designed to
alleviate transfer of the divertor geometry from the CAD system to the
\textbf{B2-Eirene} code package. However, it has proved to become a
powerful tool of more general purpose, allowing one, e. g., to specify
...
the problem geometry from scratch, and not only for B2-Eirene. With use
of this code, the most boring and time-consuming part of the
preparatory work necessary for the edge plasma modelling in a realistic
geometry, becomes really a pleasure. Try it and see yourself!

\section{DG objects}

\subsection{External objects}

The external objects are the files which DG imports or exports. They
normally have default extensions by which are used by the code when
displaying the lists of files in the I/O control windows. The following
table gives a brief description of the files used in DG.
...
in Eirene parlance). You can edit elements (create,
...
of the computational \textit{mesh} with surrounding \textit{structure}.
The separators also represent additional surfaces, but you can only
re-position their outer edges. Their purpose is separation of
\textit{additional cells} \textendash{} in terms of Eirene.
As of V. 1.4 of DG, ``separators'' have been temporarily 
disabled.
 \\
Sources
& 
...
\textendash{} and also in calculation of radiation load onto the first
wall.
 \\
Equilibrium
 &  
A simple visualisation of the equilibrium data \textendash{} red and
blue dashes corresponding to negative and positive, respectively, 
values of the poloidal
magnetic flux.
 \\
Mesh
 & 
Computational mesh, displayed in blue, 
produced with a grid generator (Sonnet or 
Carre) and imported into DG.
Allows you to check the quality of the mesh and to
make the necessary corrections to the input data for the grid
generator.
 \\
Chords
 & { 
Straight line segments representing the diagnostic chords for Eirene
and b2plot.
} \\
% {
% COMMENTED THIS OUT SINCE I DON'T THINK ONE CAN HAVE CELLS WITHOUT
% SEPARATORS & SEPARATORS ARE NOT PRESENTLY AVAILABLE. 
% Cells
% } & { 
% Regions outside the computational mesh surrounded by the elements,
% separators, and the mesh edges (\textit{additional cells} \textendash{}
% in terms of Eirene).
% } \\
External normals
& 
Are displayed as magenta dashes and depict the orientation of the elements.
They point 
to the right as you move from the beginning to the end
of an element. This is the direction of the external normal in terms of
Eirene.
 \\
Template
 &  
Imported graphics from external source (e.g. from CAD drawing or from
old Eirene input file). The template is displayed in cyan, and it can
be converted into \textit{elements.}
\\
Axes
 &  
Co-ordinate axes which can be displayed on the drawing.
\\
Grid
 &  
...
is provided with DG.  Once the \textit{equilibrium} and topology have been
...
To start DG with an existing model (or several models) loaded, type:
\begin{quote}
\textbf{\texttt{dg
\begin{math}< \end{math}filename.dg\begin{math}> \end{math} [...]}}
\end{quote}
After you start DG, the main window will appear.
...
The following
menus are available at present;
...
in the event that DG crashes unexpectedly), you should rename this
...
mouse pointer to adjancent elements will force their normals to point to
...
number. However, when the user deletes an element, the remaining
elements are
...
into the work area. This commands ignores objects that are hidden
...
To define default values for variables in a layer, display its
...
\section{Example application of DG \& Carre with DEGAS 2}
\label{dgdegas2}

This example workflow illustrates the application of the combined
DG, Carre, and DEGAS 2's {\tt definegeometry2d} codes for a single
problem.

\begin{enumerate}
  \item {\bf Prepare equilibrium data}
  \begin{itemize}
    \item EFIT equilibrium data usually consist of 
3 files, having names beginning with 
``a'', ``g'', and ``k''.  This letter is followed by additional information
that somehow encodes the shot number or date and time during the
shot (this varies from one machine to the next). 
DG only needs the ``g'' file.
    \item Copy the desired ``g'' file to \verb+degas2/dg_carre/DG/equtrn+.  
To avoid confusion, add \verb+.efit+ to this file name.
    \item Run \verb+e2d+ on this file, e.g., \\
\verb+e2d g112811.00490.efit g112811.00490.equ+ \\
The translated \verb+.equ+ file is in a format that DG can read.
    \item Copy the resulting file to the DG run directory, 
e.g., \\
\verb+degas2/dg_carre/DG/class/nstx+.  
Moving both the \verb+.efit+ and \verb+.equ+ files 
will help prevent the {\tt equtrn} directory from getting cluttered.  E.g., \\
\verb+mv g112811.00490.e* ../class/nstx/+
  \end{itemize}
  \item {\bf Run DG}
  \begin{itemize}
    \item Change to the DG run directory.  The {\tt setup.csh} 
script creates an alias for this with the name \verb+cddg+.
    \item The DG documentation describes procedures for setting up the geometry
of an experiment from scratch.  Instead, the following describes 
modifications to an existing geometry, 
creating a new mesh from the equilibrium and
matching it up to the hardware (``elements'' in DG).  E.g., begin by
running on the command line: \\
\verb+dg nstx_25.dg+
    \item Check the 
\textbf{\texttt{Options  \begin{math}\vert{}\end{math} Output Mode...}} 
dialog to be sure that the \textbf{\texttt{Carre}} radio button is selected.
    \item Under DG's \textbf{\texttt{Edit}} menu, 
\textbf{\texttt{Delete}} the Mesh and Equilibrium.
    \item Then, use \textbf{\texttt{File \begin{math}\vert{}\end{math} Import \begin{math}\vert{}\end{math} Equilibrium}} to load the new equilibrium
file.
  \end{itemize}
  \item {\bf Create plasma mesh}
  \begin{itemize}
    \item DG is used to graphically set up the input for Carre.  Carre is run 
explicitly on the command line and the resulting mesh is loaded
into DG.
    \item The first step is to create the flux surfaces that Carre will use. 
Choose \textbf{\texttt{View \begin{math}\vert{}\end{math} Display \begin{math}\vert{}\end{math} Surfaces}} to see if there are any surfaces
already defined. 
    \begin{itemize}
      \item If none are seen, use the \textbf{\texttt{File \begin{math}\vert{}\end{math} Import \begin{math}\vert{}\end{math} Topology}} dialog
to import the topology corresponding to this equilibrium (e.g.,
SN for lower single null).
      \item If there are existing surfaces, use \textbf{\texttt{Edit \begin{math}\vert{}\end{math} Delete \begin{math}\vert{}\end{math} Surfaces}} to
delete them.
    \end{itemize}
    \item In DG's Carre mode, one uses the \textit{\texttt{Add surface}} 
mouse button to create a single surface in the core of an X-point 
equilibrium.
    \item The \textbf{\texttt{Edit \begin{math}\vert{}\end{math} Create Surface(s)...}} dialog is then used to generate the rest.
    \begin{itemize}
      \item A typical number of cells for the Core might be 26
with 19 in the SOL.  The number in the PFR must match
that of the Core. 
      \item Make a note of the number of cells used in the 
Core; this will be useful in setting up the input file for 
DEGAS 2's {\tt defineback} input file.
      \item The values of ``Delta1'' and ``Delta2'' can then be varied, 
either by hand or by tweaking the graphical display in the dialog box.
      \item The particular objective depends on the problem at hand.  E.g.,
it might be to get the spacing of surfaces
near midplane roughly uniform going across the separatrix about
1 mm apart.
    \end{itemize}
    \item Similarly, the \textbf{\texttt{Edit \begin{math}\vert{}\end{math} Create grid point(s)...}} dialog is run to 
set the spacing of the mesh along the separatrix.
    \begin{itemize}
      \item Be sure to select \textbf{\texttt{View \begin{math}\vert{}\end{math} Display \begin{math}\vert{}\end{math} Grid points}}.
      \item The dialog box works just like the one for 
\textbf{\texttt{Create surface(s)}}.
      \item Again, the number and spacing of the cells depends on the
problem at hand.
    \end{itemize}
    \item To prepare to run Carre:
    \begin{itemize}
      \item Do \textbf{\texttt{File \begin{math}\vert{}\end{math} Save}} 
to save the \verb+.dg+ file,
      \item \textbf{\texttt{File \begin{math}\vert{}\end{math} Output}} 
to create the files Carre will use,
      \item Create the links to those files using the {\tt lns} script.  The
argument for this script is the root of the DG file name.  E.g.,
for \verb+nstx_25.dg+, we would do on the command line: \\
\verb+lns nstx_25+
    \end{itemize}
    \item Run Carre
    \begin{itemize}
      \item Change to the Carre directory with the alias \verb+cdcarre+
      \item On the command line, type: \\ \verb+carre -+
      \item If all goes well, one can use the default response (in 
parentheses) to each of Carre's queries and ``y'' to the yes / no questions.
      \item Make a note of the number of the file containing the resulting
grid.  This will be stored in DG's run directory.
    \end{itemize}
    \item Load mesh into DG with the 
\textbf{\texttt{File \begin{math}\vert{}\end{math} Import \begin{math}\vert{}\end{math} Mesh}} dialog.
    \item Note that this dialog box initially looks for files ending in 
\verb+.sno+.  Replace this \verb+*.sno+ with "*" and click the 
\textbf{\texttt{Filter}} button.
  \end{itemize}
  \item {\bf Create DG's polygons}
  \begin{itemize}
    \item The first step here is to decide how much of the mesh will be used in
the DEGAS 2 simulation.  That is, {\tt definegeometry2d} allows one
to use only a contiguous subset of the entire mesh generated by Carre.
    \begin{itemize}
      \item Note that the outermost mesh cells are very narrow and can
only be discerned by zooming in.  These are used to set boundary
conditions in fluid plasma transport codes.
      \item Note also that both DG and the Carre-generated mesh file 
(this is referred to as the ``Sonnet format'' file) 
begin numbering mesh cells at 0.  In contrast, DEGAS 2's 
{\tt definegeometry2d} 
starts its numbering at 1, so add 1 to the indices noted in DG.
      \item As on the outer edge of the mesh, 
there is also a very narrow ``ghost'' cell at the
innermost edge.
    \end{itemize}
    \item The objective in defining polygons for use with 
{\tt definegeometry2d} is to have them fill the entire problem space.  
To facilitate that, begin by defining a rectangular box
(with \textbf{\texttt{Edit \begin{math}\vert{}\end{math} Create \begin{math}\vert{}\end{math} Point...}}) with
corners exactly matching the box defined by the \verb+bounds+ keyword
in {\tt definegeometry2d}.  That these match exactly is crucial.
    \item Existing polygons can be examined via the 
\textbf{\texttt{Variables \begin{math}\vert{}\end{math} Polygon \begin{math}\vert{}\end{math} (polygon number)}} dialogs. 
    \begin{itemize}
      \item The ``Wall'' component of the polygon 
refers to a set of contiguous elements that will form most or 
all of the polygon.  The \textbf{\texttt{Mark}} 
button can be used to show the current definition.  Its value can 
be changed by marking or unmarking elements and then clicking the 
\textbf{\texttt{Set}} button.
      \item The \textbf{\texttt{Material}} 
component is currently not used and can be ignored.
    \end{itemize}
    \item Polygons consisting only of a ``Wall'' (closed polygons) 
are complete.
    \begin{itemize}
      \item The ``First Mesh Connection'' and ``Second Mesh Connection'' 
components need not be specified.
    \end{itemize}
    \item However, some polygons will be closed by connecting the 
``Wall'' elements to an edge of the mesh and following along that edge.  
The points at which these connections are made and the edge to be included are
specified by the ``First Mesh Connection'' and ``Second Mesh Connection'' 
components and the \verb+iedge+ keyword in {\tt definegeometry2d}.
    \begin{itemize}
      \item Each ``Mesh Connection'' consists of the two mesh elements 
adjacent to the desired connection point, one going along the
flux surface and one along the orthogonal.  If the 
\verb+sonnet_mesh+ keyword in {\tt definegeometry2d} 
will use a subset of the mesh, these elements need to be in that subset.
      \item  Make a note of the corresponding mesh edge to use with the
\verb+iedge+ keyword in {\tt definegeometry2d}.  These are denoted by
S (for ``south'', $i_z = 0$), E (``east'', $i_x = n_x$), N 
(``north'', $i_z = n_z$), or W (``west'', $i_x = 0$).
    \end{itemize}
  \end{itemize}
  \item {\bf Finish with DG}
  \begin{itemize}
    \item Again, save the file and do 
\textbf{\texttt{File \begin{math}\vert{}\end{math} Output}}.
    \item Copy the resulting \verb+.dgo+ (not \verb+.dg+) and Carre mesh
files to the DEGAS 2 run directory.
  \end{itemize}
\end{enumerate}
