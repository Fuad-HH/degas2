...
class DG2D:
    ...
    def write_wallfile(self):
        ...
        for i in range(0,Nnode):
            wallfile.write("%f   %f \n"%(self.nodes[i].coords[0],self.nodes[i].coords[1]))
        wallfile.close()
    ...
    def write_files(self):
        ...
        for i in range(0,Nnew):
            self.nodes.append(newvertices[i])

        for poly in self.aux_polys:
            self.write_aux_polygon(poly)

        f = open(self.infile_name,"a") 
        outpoly.close_in_universal_cell(f,outpoly.vertices,1,self.current_stratum+1,material,recyc,walltemp=walltemp,clockwise=True)
        # Finally, close in
