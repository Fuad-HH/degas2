...
#define EAT_AUTO_SEMI {if(auto_semi && next_byte() != ';') BACK_UP}
#define BACK_UP {saved_token = NO; cur_byte -= last_bytes;} \

#define NOT_BEFORE (eight_bits)'\177' /* Make octal constant. */
...
#define IS_WHITE(a) ((a) == ' ' || (a) == tab_mark) \
...
#define isAupper(c) (c<='Z' && c>='A')
#define isAlower(c) (c<='z' && c>='a') \
...
#define isBdigit(c) ((c)=='0' || (c)=='1') /* Is it a binary numeral? */
#define isOdigit(c) (isDigit(c) && c!='8' && c!='9') /* Octal numeral? */
...
#define dot_const (eight_bits)'\23' /* Make octal constant. */ \

#define MAKE_RECURSIVE '*' /* To allow a WEB macro to be recursive, preface its 
\
	definition by this symbol, as in `\.{@m *R R}'.  (Recursive macros \
	are not presently implemented.) */ \

#define AUTO_INSERT '['
#define END_AUTO_INSERT ']' \

#define PROTECTED '!' /* A protected macro ---`\.{@m!}'---may not be redefined. 
*/ \
...
value. */
#define neqv (eight_bits)'\10' /* Make octal constant. */ /* `|@r .neqv.|'.*/
#define ellipsis neqv /* `|...|'.*/ \

#define stmt_label (eight_bits)'\30' /* Make octal constant. */ \

#define slash_slash (eight_bits)'\26' /* Make octal constant. */  /* 
Concatenation `|@r \/|' .*/ \

#define bell (eight_bits)'\7' /* Make octal constant. */ /* |ASCII| code for 
ringing the bell.*/
#define tab_mark '\t' /* |ASCII| code used as tab-skip.*/
#define line_feed (eight_bits)'\12' /* Make octal constant. */ /* |ASCII| code 
thrown away at end of line; $\equiv$ \
			\.{'\\n'}. */
#define form_feed (eight_bits)'\14' /* Make octal constant. */ /* |ASCII| code 
used at end of page.*/
#define carriage_return (eight_bits)'\15' /* Make octal constant. */ /* |ASCII| 
code used at end of line.*/ \

#define gt_gt (eight_bits)'\20' /* Make octal constant. */ /* `|>>|'; this 
doesn't exist in MIT.*/
#define lt_lt (eight_bits)'\22' /* Make octal constant. */ /* `|<<|'; this 
doesn't exist in MIT.*/ \

#define plus_plus (eight_bits)'\13' /* Make octal constant. */ /* `|++|'; this 
corresponds to MIT's up-arrow.*/
#define minus_minus (eight_bits)'\1' /* Make octal constant. */ /* `|--|'; this 
corresponds to MIT's down-arrow.*/ \

#define minus_gt (eight_bits)'\31' /* Make octal constant. */ /* `|->|'.*/
#define eqv minus_gt /* `|@r .eqv.|'.*/ \

#define not_eq (eight_bits)'\32' /* Make octal constant. */ /* `|!=|'.*/ \

#define paste (eight_bits)'\33' /* Make octal constant. */ /* `|##|'.*/ \

#define lt_eq (eight_bits)'\34' /* Make octal constant. */ /* `|<=|'.*/
#define gt_eq (eight_bits)'\35' /* Make octal constant. */ /* `|>=|'.*/ \

#define eq_eq (eight_bits)'\36' /* Make octal constant. */ /* `|==|'.*/ \

#define or_or (eight_bits)'\37' /* Make octal constant. */ /* `||| |'.*/ \

#define begin_language (eight_bits)'\16' /* Make octal constant. */ /* Mark a 
language switch.*/ \

#define left_array (eight_bits)'\21' /* Make octal constant. */  /* `$\LS$'.*/
#define right_array (eight_bits)'\25' /* Make octal constant. */ /* `$\SR$'.*/ 
\

#define interior_semi (eight_bits)'\24' /* Make octal constant. */ /* `\.;'.  
In \Fortran, a semicolon that's already \
...
#define begin_format_stmt (eight_bits)'\171' /* Make octal constant. */
#define end_format_stmt (eight_bits)'\172' /* Make octal constant. */ \

#define is_identifier(c) (isAlpha(c) || c=='_' || c=='$' || \
	(c=='%' && !C_LIKE(language) && !Fortran88) ) \
		/* This defines the starting character of an identifier. */ \

#define is_kind(c) (isDigit(c) || isAlpha(c) || c=='_' || c=='$') \
