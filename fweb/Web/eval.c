 ...
 created with UNIX on "Friday, September 25, 1998 at 8:02." \
  COMMAND LINE: "./ftangle ./eval -uCONFIG -mCONFIG -mGCC -= eval.c"\
...
#define DEFINED_TOKEN  (eight_bits)'\23'
#define UNARY_MINUS  (eight_bits)'\24' \
...
expression")),YES,a>=' '?a:'?',a);longjmp(top_of_evaluate,1);
}
break;
}
...
case '*':ARITH(*);
case '/':chk_zero('/',val1);ARITH(/);
case '%':chk_zero('%',val1);BIT(%);
