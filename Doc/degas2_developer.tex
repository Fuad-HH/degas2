...
energy source and loss rates are evaluted, as are the
...
constiuent tallies via {\tt outputbrowser}). 
...
(and the rarely used, arbitary factor {\tt so\_scale}), and 
...
region of the problem; this will be discussed in Sec. XX
...
orders, examing the scaling of the errors
with the step size.
...
{\tt defineback}.  As with virtually all
...
the axisymmetric values.

We only need the axisymmetric file  in producing the neutral background,
so this initial 
...
in both the initial neutral background run
...
Note that {\tt defineback} does not provide a means
of setting the restart flag via its input file.  Here,
we change the value of {\tt so\_restart} from its
default value of $0$ to $1$.  

% defineback XGC_GPI_19_ex/xgc_gpi_19_db_3D_ss
...
For {\tt flighttest}, parallelization can be
turned on with \verb+MPI = yes+.  Note that
\verb+MASTER_SAMPLE+ should be left with the
default \verb+no+, except in time dependent mode.
