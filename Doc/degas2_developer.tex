...
\item The initial call to {\tt defineback} will take longer
  ...
  ({\tt xgc1\_int\_19.nc}; see Sec.~\ref{fieldline}) will
...
The number of flights needed to adequately
populate the snapshot distribution
has already been
set in this input file. 
...
Next, the user need only run {\tt flighttest} to create
the snapshot distribution.  To prevent the snapshot file from
being accidently overwritten, it has been copied to
{\tt sn\_xgc1\_cmod\_init.nc}.  Should the user want
to restart the time dependent run from the beginning,
it can be copied back to the {\tt sn\_xgc1\_cmod.nc}
pointed to by the {\tt degas2.in} file.  The output
netCDF file from this initialization run can be used
to check the statistics, etc., but is otherwise not
needed for the rest of the run.

\subsubsection{Improving Statistics}
\label{statistics}

Before beginning the actual time dependent run,
we need to examine
...
always perform time dependent runs of
{\tt flighttest} compiled with
...
The batch script {\tt gpi\_script\_20} illustrates
the practical application of this tactic.  The
user will need to adapt the details according to
the size of the run to be performed and available
resources.
