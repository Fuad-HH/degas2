...
def write_dg2d_input_from_wallfile(wallfile_name,topology,topology_params,dg2dfile_name="dg2d.in",debug=False):
    ...
    if topology == "LTX":
        ...
        for vertex in wall[0].vertices[limiter_vertex_idx[0]:-1]:
            polys[num_closed_surfaces].add_vertex(vertex)
        for vertex in wall[0].vertices[0:limiter_vertex_idx[1]]:
            polys[num_closed_surfaces].add_vertex(vertex)
        # The relevant part of the LCFS, in reverse order
        for vertex in wall[1].vertices[limlcfs_idx_lower::-1]:
            polys[num_closed_surfaces].add_vertex(vertex)
        for vertex in wall[1].vertices[-1:limlcfs_idx_upper:-1]:
            polys[num_closed_surfaces].add_vertex(vertex)
        polys[num_closed_surfaces].add_vertex(wall[0].vertices[limiter_vertex_idx[0])
        ...
        for vertex in wall[1].vertices[limlcfs_idx_lower:limlcfs_idx_upper+1]:
        ...
        for vertex in wall[-1].vertices(-1::-1):
            dummyPoly.add_vertex(vertex)
        wallidx_intersect_lower = dummyPoly.get_next_vertex_extrapolated_to_wall(wall[0])
        dummyPoly = Polygon(increment=False)
        for vertex in wall[-1].vertices:
            dummyPoly.add_vertex(vertex)
        wallidx_intersect_upper = dummyPoly.get_next_vertex_extrapolated_to_wall(wall[0])

        # Add relevant part of upper solid wall
        for vertex in wall[0].indices[limiter_vertex[0]:wallidx_intersect_upper-1:-1]:
            polys[num_closed_surfaces+1].add_vertex(vertex)

        # Add first open flux surface, in reverse order
        for vertex in wall[-1].indices[-1::-1]:
            polys[num_closed_surfaces+1].add_vertex(vertex)

        # Add relevant part of lower solid wall
        for vertex in wall[0].indices[limiter_vertex[1]:wallidx_intersect_lower-1:-1]:
            polys[num_closed_surfaces+1].add_vertex(vertex)

        # Close the polygon
        polys[num_closed_surfaces+1].add_vertex(wall[1].vertices[limlcfs_idx_lower])
        ...
        for vertex in wall[num_closed_surfaces].vertices():
            polys[num_closed_surfaces+2].add_vertex(vertex)

        # Now, find the solid wall index corresponding to the ends of the first open flux surfaces
        wallidx_intersect_lower = polys[num_closed_surfaces+2].get_next_vertex_extrapolated_to_wall(wall[0])

        # First build a dummy polygon to track other surface
        dummyPoly = Polygon(increment=False)
        for vertex in wall[0].vertices(-1::-1):
            dummyPoly.add_vertex(vertex)
        wallidx_intersect_upper = dummyPoly.get_next_vertex_extrapolated_to_wall(wall[0])

        # Add relevant part of upper solid wall
        for vertex in wall[0].indices[wallidx_intersect_lower:wallidx_intersect_upper+1]:
            polys[num_closed_surfaces+2].add_vertex(vertex)

        # Close the polygon
        polys[num_closed_surfaces+2].add_vertex(wall[num_closed_surfaces].vertices[0])
        ...
        for ipoly in range(num_closed_surfaces+2,num_closed_surfaces+2+num_open_surfaces)
            ...
            for vertex in wall[opensurf_idx].vertices:
                polys[ipoly].add_vertex(vertex)
            wallidx_intersect_bl = polys[ipoly].get_next_vertex_extrapolated_to_wall(wall[0])
            
            dummyPoly = Polygon(increment=False)
            for vertex in wall[opensurf_idx-1].vertices:
                dummyPoly.add_vertex(vertex)
            wallidx_intersect_br = dummyPoly.get_next_vertex_extrapolated_to_wall(wall[0])

            for vertex in wall[0].vertices[wallidx_intersect_bl:wallidx_intersect_br+1]:
                polys[ipoly].add_vertex(vertex)

            for vertex in wall[opensurf_idx-1].vertices[-1::-1]:
                polys[ipoly].add_vertex(vertex)

            wallidx_intersect_ur = polys[ipoly].get_next_vertex_extrapolated_to_wall(wall[0])

            dummyPoly = Polygon(increment=False)
            for vertex in wall[opensurf_idx].vertices[-1::-1]:
                dummyPoly.add_vertex(vertex)
            wallidx_intersect_ul = dummyPoly.get_next_vertex_extrapolated_to_wall(wall[0])

            for vertex in wall[0].vertices[wallidx_intersect_ur:wallidx_intersect_ul-1:-1]:
                polys[ipoly].add_vertex(vertex)

            polys[ipoly].add_vertex(wall[opensurf_idx].vertices[0])
